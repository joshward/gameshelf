...
export class XmlElement {
  ...
  public getAttribute(key: string, options?: { optional?: boolean }): XmlValue | undefined {
    ...
    const attributes = safeGetValue(this.element, '_attributes')
    const attribute = safeGetValue(attributes, key)
  ...
  public getText(options?: { optional?: boolean }): XmlValue | undefined {
    ...
    const text = safeGetValue(this.element, '_text')
    if (text === undefined) {
      if (options?.optional) {
        return undefined
      }
  ...
  public getNode(key: string): XmlNode {
    return new XmlNode(safeGetValue(this.element, key), `${this.name}.${key}`)
  }
...
export class XmlReader {
  private constructor(private root: unknown) {}

  public static readonly parse = async (xml: string): Promise<XmlReader> => {
  ...
  public readonly getNode = (key: string): XmlNode =>
    new XmlNode(safeGetValue(this.root, key), key);
}
